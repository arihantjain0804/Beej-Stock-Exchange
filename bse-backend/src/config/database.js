...
const needsSSL = process.env.DB_SSL === 'false'
  ? false
  : (process.env.DB_SSL === 'true' || dbHost.includes('supabase.co'));
const pool = new Pool({ host: dbHost, port: parseInt(process.env.DB_PORT) || 5432, database: process.env.DB_NAME || 'bse_db', user: process.env.DB_USER || 'bse_user', password: process.env.DB_PASSWORD, min: parseInt(process.env.DB_POOL_MIN) || 2, max: parseInt(process.env.DB_POOL_MAX) || 10, idleTimeoutMillis: 30000, connectionTimeoutMillis: 5000, ssl: needsSSL ? { rejectUnauthorized: false } : false });
