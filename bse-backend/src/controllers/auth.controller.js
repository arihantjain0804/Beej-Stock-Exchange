...
const getMe = async (req, res) => { const { rows } = await query(`SELECT u.id,u.phone,u.email,u.full_name,u.role,u.kyc_status,u.wallet_balance,u.created_at,fp.farm_name,fp.state,fp.fpo_name FROM users u LEFT JOIN farmer_profiles fp ON fp.user_id=u.id WHERE u.id=$1`, [req.user.sub]); if (!rows.length) return unauthorized(res); const u = rows[0]; return success(res, { id:u.id, phone:u.phone, email:u.email, full_name:u.full_name, role:u.role, kyc_status:u.kyc_status, wallet_balance:parseFloat(u.wallet_balance), created_at:u.created_at }); };
module.exports = { sendOTPValidation, sendOTP, verifyOTPValidation, verifyOTP, refreshToken, getMe };